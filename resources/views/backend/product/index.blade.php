      ...
      <div class="table-responsive">
        ...
        <div class="phanTrang">
            <button onclick="window.location.href='{{ $products->previousPageUrl() }}'" 
                    class="phanTrang-btn" 
                    {{ $products->onFirstPage() ? 'disabled' : '' }}>←</button>
            
            @for ($i = 1; $i <= $products->lastPage(); $i++)
                <button onclick="window.location.href='{{ $products->url($i) }}'" 
                        class="phanTrang-number {{ $products->currentPage() == $i ? 'active' : '' }}">
                    {{ $i }}
                </button>
            @endfor
            ...
            <button onclick="window.location.href='{{ $products->nextPageUrl() }}'" 
                    class="phanTrang-btn" 
                    {{ $products->currentPage() == $products->lastPage() ? 'disabled' : '' }}>→</button>
        </div>
        @else
...
@push('styles')
  ...
  <style>
      ...
      /* Ẩn phân trang mặc định của DataTables */
      div.dataTables_wrapper div.dataTables_paginate {
          display: none;
      }
      
      /* Style phân trang từ trang order */
      .phanTrang {
          margin: 20px 0;
          display: flex;
