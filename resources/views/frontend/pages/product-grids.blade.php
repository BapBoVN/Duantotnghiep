...
@section('title','Little Brain || Sản phẩm')
...
@section('main-content')
                                ...
                                <div class="single-widget recent-post">
                                    ...
                                    @foreach($recent_products as $product)
                                        ...
                                        <div class="single-post first">
                                            <div class="image">
                                                <img src="{{$photo[0]}}" alt="{{$photo[0]}}" style="width: 70px; height: 100px; object-fit: cover; border-radius: 0;  margin-top: -15px;">
                                            </div>
                                            <div class="content">
    ...
    <a href="{{ $products->previousPageUrl() }}" class="prev @if(!$products->onFirstPage()) @else disabled @endif">
        &laquo; <!-- Mũi tên trái -->
    </a>
    ...
    @if ($startPage > 1)
        ...
        @if ($startPage > 2)
            <span>...</span> <!-- Dấu ba chấm nếu cần -->
        @endif
    @endif

    @for ($i = $startPage; $i <= $endPage; $i++)
        <a href="{{ $products->url($i) }}" class="@if($currentPage == $i) active @endif">
            {{ $i }}
        </a>
    @endfor
    ...
    <a href="{{ $products->nextPageUrl() }}" class="next @if(!$products->hasMorePages()) @else disabled @endif">
        &raquo; <!-- Mũi tên phải -->
    </a>
</div>
                                        ...
                                        <div class="quickview-content">
                                            ...
                                            <div class="quickview-ratting-review">
                                                <div class="quickview-ratting-wrap">
                                                    ...
                                                    <a href="#"> ({{$rate_count}} Đánh giá chung)</a>
                                                </div>
                                                <div class="quickview-stock">
                                                    @if($product->stock >0)
                                                    <span><i class="fa fa-check-circle-o"></i> Còn lại: {{$product->stock}} sản phẩm</span>
                                                    @else
                                                    <span><i class="fa fa-times-circle-o text-danger"></i> {{$product->stock}} Đã hết</span>
                                                    @endif
                                                </div>
                                            </div>
                                            @php
                                                $after_discount=($product->price-($product->price*$product->discount)/100);
                                            @endphp
                                            <h3><small><del class="text-muted">{{number_format($product->price)}} VND</del></small>    {{number_format($after_discount)}} VND </h3>
...
.pagination a {
    display: inline-block; /* Đặt các phần tử thành inline-block */
...
.pagination a.disabled {
    color: #6c757d; /* Màu chữ cho nút vô hiệu hóa */
    background-color: #f8f9fa; /* Giữ màu nền nhẹ */
    border: 1px solid #dee2e6; /* Giữ viền nhẹ */
